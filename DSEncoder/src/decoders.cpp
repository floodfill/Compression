...
//	if ((*end != '\0') || (decID < 0) || (decID >= NUMDECODERS)
//			|| (errno == ERANGE))
//		__usage("DecoderID '%s' invalid", argv[1]);
//
//	shared_ptr<uint32_t> __list(new uint32_t[OUTPUTMEM(MAXLEN)],
...
//	uint32_t *sbuf = NULL;
//
//	shared_ptr<uint32_t> __sbuf(static_cast<uint32_t *>(0),
//			default_delete<uint32_t[]>());
//
//	if (decID == D_VSEREST || decID == D_VSEHYB) {
//		shared_ptr<uint32_t> __temp(new uint32_t[OUTPUTMEM(MAXLEN)],
//				default_delete<uint32_t[]>());
//
...
//		strcat(ofile, DECEXT);
//
//		shared_ptr<FILE> __temp(fopen(ofile, "w"), __deleter_fclose);
...
//
//		if (decID == D_VSEREST || decID == D_VSEHYB) {
//			memcpy(sbuf, ptr, sizeof(uint32_t) * (next_pos - cmp_pos));
//			ptr = sbuf;
//		}
//
//		/* Do decoding */
...
//		uint32_t _num = num - 1;
//		(decoders[decID])(ptr, next_pos - cmp_pos, list, _num);
//		dtime += __get_time() - tm;
//
...
//	cout << "Performance: " << (dints + 0.0) / (dtime * 1000000) << " mis"
//			<< endl;
//	cout << "Size: " << (sum_sizes / 1024) * 4 << " KiB" << endl;
//	cout << "Size: " << ((sum_sizes + 0.0) / (dints + 0.0)) * 32 << " bpi"
//			<< endl;
//
//	return EXIT_SUCCESS;
//}
